--[[
=======================================================
This code is based on/copied from Krock's "bitchange" mod
...
local function get_exchange_shop_tube_config(mode)
	if mode == "groups" then
		if wow.exchangeshop_pipeworks then
...
minetest.register_on_player_receive_fields(function(sender, formname, fields)
	if formname ~= "wow:shop_formspec" then
		return
	end
	...
	if fields.title and wow.has_access(shop_owner, player_name) then
		-- Limit title length
		...
		if title ~= fields.title then
			if fields.title ~= "" then
				...
				meta:set_string("infotext", "Wownero shop (owned by "
					.. shop_owner ..")")
			end
	...
	if fields.exchange then
		...
		-- Throw error message
		if err_msg ~= "" then
			minetest.chat_send_player(player_name, "Wownero shop: "..err_msg)
		end
	elseif wow.has_access(shop_owner, player_name) then
		local mode
		...
		minetest.show_formspec(player_name, "wow:shop_formspec",
			get_exchange_shop_formspec(mode, pos, title))
	end
end)

minetest.register_node(":wow:shop", {
	description = "Shop",
	...
	--sounds = default.node_sound_wood_defaults(), TODO
	...
	on_construct = function(pos)
		local meta = minetest.get_meta(pos)
		meta:set_string("infotext", "Wownero shop (constructing)")
		...
		inv:set_size("cust_ow", 1) -- owner wants
	...
	can_dig = function(pos,player)
		...
		minetest.chat_send_player(player:get_player_name(),
			"Cannot dig wownero shop: one or multiple stocks are in use.")
		return false
	end,
	on_rightclick = function(pos, node, clicker, itemstack)
		...
		local mode = "customer"
		if wow.has_access(meta:get_string("owner"), player_name) and
	...
	allow_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
		local meta = minetest.get_meta(pos)
		if wow.has_access(meta:get_string("owner"), player:get_player_name()) then
			return count
		end
		return 0
	end,
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		...
		local meta = minetest.get_meta(pos)
		if wow.has_access(meta:get_string("owner"), player:get_player_name())
