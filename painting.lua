...
minetest.register_node("wow:pic", {
	description = "Picture",
	tiles = { "white.png" },
	...
	after_dig_node = function(pos, oldnode, oldmetadata, digger)
		...
		for _,e in pairs(minetest.get_objects_inside_radius(pos, 0.5)) do
			...
			if e:get_luaentity()
			and e:get_luaentity().name == "wow:picent" then
				e:remove()
			end
		end
		...
		if inv
		and inv:room_for_item("main", "wow:paintedcanvas") then

			-- put picture data back into inventory as item
			inv:add_item("main", {
				name = "wow:paintedcanvas",
				count = 1,
				metadata = oldmetadata.fields["wow:picturedata"]
			})
...
minetest.register_entity("wow:picent", {
	...
	on_activate = function(self, staticdata)
		...
		local data = meta:get_string("wow:picturedata")
...
minetest.register_entity("wow:paintent", {
	collisionbox = { 0, 0, 0, 0, 0, 0 },
	visual = "upright_sprite",
	textures = { "white.png" },
...
minetest.register_craftitem("wow:paintedcanvas", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		minetest.add_node(pos, {name = "wow:pic", param2 = fd})

		--save metadata
		local data = itemstack:get_metadata()

		minetest.get_meta(pos):set_string("wow:picturedata", data)
		...
		local p = minetest.add_entity(pos, "wow:picent"):get_luaentity()
...
for i = 4, 6 do
	...
	minetest.register_craftitem("wow:canvas_" .. 2 ^ i, {
...
minetest.register_node("wow:canvasnode", {
	description = "Canvas",
	tiles = { "white.png" },
	inventory_image = "painted.png",
	drawtype = "nodebox",
...
minetest.register_node("wow:easel", {
	description = "Easel",
	...
	on_punch = function(pos, node, player)
		...
		minetest.add_node(pos, { name = "wow:canvasnode", param2 = fd})
		...
		local p = minetest.add_entity(pos, "wow:paintent"):get_luaentity()
...
for color, _ in pairs(textures) do
	...
	minetest.register_tool("wow:brush_" .. color, brush_new)
...
minetest.register_alias("easel", "wow:easel")
minetest.register_alias("canvas", "wow:canvas_16")
